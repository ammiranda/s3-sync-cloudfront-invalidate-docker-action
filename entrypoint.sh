...
if [ -z "$AWS_CLOUDFRONT_DISTRO_ID" ]; then
  echo "AWS_CLOUDFRONT_DISTRO_ID is not set. Quitting."
  exit 1
fi

if [ -z "$AWS_CLOUDFRONT_PATH" ]; then
  echo "AWS_CLOUDFRONT_PATH is not set. Quitting."
  exit 1
fi
